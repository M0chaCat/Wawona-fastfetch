...
pkgs.stdenv.mkDerivation {
  ...
  preConfigure = ''
    ...
    [built-in options]
    c_args = ['-arch', 'arm64', '-isysroot', '$SDKROOT', '-mios-simulator-version-min=15.0', '-fPIC']
    cpp_args = ['-arch', 'arm64', '-isysroot', '$SDKROOT', '-mios-simulator-version-min=15.0', '-fPIC']
    c_link_args = ['-arch', 'arm64', '-isysroot', '$SDKROOT', '-mios-simulator-version-min=15.0']
    cpp_link_args = ['-arch', 'arm64', '-isysroot', '$SDKROOT', '-mios-simulator-version-min=15.0']
    EOF
  '';
  configurePhase = ''
