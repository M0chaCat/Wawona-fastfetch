 ...
 * and cursor drawing. The primary Vulkan lifecycle (instance, device, swapchain)
 * is managed in android_jni.c.
 */
...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "WawonaRenderer", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "WawonaRenderer", __VA_ARGS__)
...
static VkFormat shm_format_to_vk(uint32_t wl_format) {
    /* WL_SHM_FORMAT_ARGB8888 = 0, XRGB8888 = 1 - both 32bpp BGRA in memory on little-endian */
    (void)wl_format;
...
/* Create the quad pipeline - call from android_jni after device and render pass exist */
int renderer_android_create_pipeline(VkDevice device,
                                      VkPhysicalDevice physical_device,
                                      VkRenderPass render_pass,
    ...
    res = vkCreateDescriptorSetLayout(device, &dsl_ci, NULL, &s_renderer->descriptor_set_layout);
    if (res != VK_SUCCESS) {
    ...
    res = vkCreatePipelineLayout(device, &pl_ci, NULL, &s_renderer->pipeline_layout);
    if (res != VK_SUCCESS) {
        LOGE("Failed to create pipeline layout: %d", res);
        vkDestroyDescriptorSetLayout(device, s_renderer->descriptor_set_layout, NULL);
        vkDestroyShaderModule(device, frag_module, NULL);
        vkDestroyShaderModule(device, vert_module, NULL);
        goto err;
    }
    ...
    res = vkCreateDescriptorPool(device, &dp_ci, NULL, &s_renderer->descriptor_pool);
    if (res != VK_SUCCESS) {
    ...
    res = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &gp_ci, NULL, &s_renderer->pipeline);
    vkDestroyShaderModule(device, vert_module, NULL);
    vkDestroyShaderModule(device, frag_module, NULL);
    if (res != VK_SUCCESS) {
    ...
    res = vkAllocateMemory(device, &mem_ai, NULL, &s_renderer->vertex_buffer_memory);
    if (res != VK_SUCCESS) {
    ...
    vkBindBufferMemory(device, s_renderer->vertex_buffer, s_renderer->vertex_buffer_memory, 0);

    /* Map and upload vertex data */
    void *ptr;
    res = vkMapMemory(device, s_renderer->vertex_buffer_memory, 0, VK_WHOLE_SIZE, 0, &ptr);
    if (res == VK_SUCCESS) {
        memcpy(ptr, g_quad_vertices, sizeof(g_quad_vertices));
        memcpy((char *)ptr + sizeof(g_quad_vertices), g_quad_indices, sizeof(g_quad_indices));
...
void renderer_android_destroy_pipeline(void) {
    if (!s_renderer || !s_renderer->device) return;

    VkDevice dev = s_renderer->device;
    for (int i = 0; i < MAX_CACHED_BUFFERS; i++) {
        CachedTexture *t = &s_renderer->cache[i];
        if (t->image_view != VK_NULL_HANDLE) vkDestroyImageView(dev, t->image_view, NULL);
        if (t->image != VK_NULL_HANDLE) vkDestroyImage(dev, t->image, NULL);
        if (t->memory != VK_NULL_HANDLE) vkFreeMemory(dev, t->memory, NULL);
    }
    if (s_renderer->vertex_buffer != VK_NULL_HANDLE) vkDestroyBuffer(dev, s_renderer->vertex_buffer, NULL);
    if (s_renderer->vertex_buffer_memory != VK_NULL_HANDLE) vkFreeMemory(dev, s_renderer->vertex_buffer_memory, NULL);
    if (s_renderer->pipeline != VK_NULL_HANDLE) vkDestroyPipeline(dev, s_renderer->pipeline, NULL);
    if (s_renderer->descriptor_pool != VK_NULL_HANDLE) vkDestroyDescriptorPool(dev, s_renderer->descriptor_pool, NULL);
    if (s_renderer->sampler != VK_NULL_HANDLE) vkDestroySampler(dev, s_renderer->sampler, NULL);
    if (s_renderer->pipeline_layout != VK_NULL_HANDLE) vkDestroyPipelineLayout(dev, s_renderer->pipeline_layout, NULL);
    if (s_renderer->descriptor_set_layout != VK_NULL_HANDLE) vkDestroyDescriptorSetLayout(dev, s_renderer->descriptor_set_layout, NULL);
...
int renderer_android_cache_buffer(VkCommandBuffer cmd_buf,
                                  uint64_t buffer_id, uint32_t width, uint32_t height,
                                  uint32_t stride, uint32_t format,
                                  const uint8_t *pixels, size_t size) {
    if (!s_renderer || !pixels || width == 0 || height == 0) return -1;
    ...
    /* If reusing, destroy old resources */
    if (slot->image_view != VK_NULL_HANDLE) vkDestroyImageView(dev, slot->image_view, NULL);
    if (slot->image != VK_NULL_HANDLE) vkDestroyImage(dev, slot->image, NULL);
    if (slot->memory != VK_NULL_HANDLE) vkFreeMemory(dev, slot->memory, NULL);
    ...
    res = vkCreateBuffer(dev, &staging_ci, NULL, &staging_buf);
    if (res != VK_SUCCESS) {
    ...
    for (uint32_t i = 0; i < mem_props_staging.memoryTypeCount; i++) {
        if ((staging_req.memoryTypeBits & (1u << i)) &&
            (mem_props_staging.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)) {
            staging_type = i;
            break;
        }
    }
    ...
    res = vkAllocateMemory(dev, &staging_mem_ai, NULL, &staging_mem);
    if (res != VK_SUCCESS) {
    ...
    void *map_ptr;
    res = vkMapMemory(dev, staging_mem, 0, buffer_size, 0, &map_ptr);
    if (res != VK_SUCCESS) {
    ...
    vkCmdCopyBufferToImage(cmd_buf, staging_buf, slot->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
VkImageView renderer_android_get_texture(uint64_t buffer_id) {
    if (!s_renderer) return VK_NULL_HANDLE;
    for (int i = 0; i < MAX_CACHED_BUFFERS; i++) {
        if (s_renderer->cache[i].buffer_id == buffer_id && s_renderer->cache[i].image_view != VK_NULL_HANDLE)
...
static void get_texture_size(uint64_t buffer_id, uint32_t *width, uint32_t *height) {
    *width = 0;
    *height = 0;
...
static void reset_descriptor_pool(void) {
    if (!s_renderer || s_renderer->descriptor_pool == VK_NULL_HANDLE) return;
...
void renderer_android_draw_quads(VkCommandBuffer cmd_buf,
                                 ...
                                 uint32_t extent_width,
                                 uint32_t extent_height) {
    if (!s_renderer || !nodes || node_count == 0) return;

    reset_descriptor_pool();

    vkCmdBindPipeline(cmd_buf, VK_PIPELINE_BIND_POINT_GRAPHICS, s_renderer->pipeline);
    vkCmdBindVertexBuffers(cmd_buf, 0, 1, &s_renderer->vertex_buffer, (VkDeviceSize[]){0});
    vkCmdBindIndexBuffer(cmd_buf, s_renderer->vertex_buffer, sizeof(g_quad_vertices),
    ...
    for (size_t i = 0; i < node_count; i++) {
        ...
        /* Use a descriptor set - we need to create/update per draw or use a simpler path.
         * Vulkan requires a descriptor set for the sampler. We can use
        ...
        VkResult res = vkAllocateDescriptorSets(s_renderer->device, &alloc_info, &set);
...
void renderer_android_draw_cursor(VkCommandBuffer cmd_buf,
                                  ...
                                  float cursor_hotspot_x, float cursor_hotspot_y,
                                  uint32_t extent_width,
                                  uint32_t extent_height) {
    if (!s_renderer || cursor_buffer_id == 0) return;
