...
static void params_create_common(struct wl_client *client,
  ...
  // If multi-planar support is needed, we would need to handle it in
  // metal_dmabuf
  if (params->n_planes > 1) {
    printf("Warning: Multi-planar dmabuf not fully supported yet, using plane "
           "0\n");
  }
  ...
  // Duplicate FD because import takes ownership or we need to keep it if import
  // fails? metal_dmabuf_import closes the FD. So we should duplicate if we want
  // to keep params valid? But params are destroyed after create usually.
...
static void params_create_immed(struct wl_client *client,
                                ...
                                int32_t height, uint32_t format,
                                uint32_t flags) {
  params_create_common(client, resource, buffer_id, width, height, format,
                       flags);
}
...
static void dmabuf_create_params(struct wl_client *client,
  ...
  wl_resource_set_implementation(params_resource, &params_interface, params,
...
zwp_linux_dmabuf_v1_create(struct wl_display *display) {
  ...
  impl->global = wl_global_create(display, &zwp_linux_dmabuf_v1_interface, 3,
                                  impl, bind_dmabuf);
