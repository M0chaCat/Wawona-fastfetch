...
void wawona_log(const char *module, const char *format, ...)
{
    ...
    if (strstr(format, "❌") || strstr(format, "ERROR")) priority = ANDROID_LOG_ERROR;
    else if (strstr(format, "⚠️") || strstr(format, "WARN")) priority = ANDROID_LOG_WARN;
    else if (strstr(format, "🔍") || strstr(format, "DEBUG")) priority = ANDROID_LOG_DEBUG;
...
void log_printf(const char *prefix, const char *format, ...)
{
    ...
    while (*src && dst_len < sizeof(clean_prefix) - 1) {
        if (*src != '[' && *src != ']') {
            ...
            if (*src == ' ' && (dst == clean_prefix || *(src + 1) == '\0' || *(src + 1) == ' ')) {
                src++;
                continue;
            }
            *dst++ = *src;
    ...
    *dst = '\0';
    ...
    va_start(args, format);
    int priority = ANDROID_LOG_INFO;
    if (strstr(prefix, "ERROR") || strstr(format, "❌")) priority = ANDROID_LOG_ERROR;
    else if (strstr(prefix, "WARN") || strstr(format, "⚠️")) priority = ANDROID_LOG_WARN;
    else if (strstr(prefix, "DEBUG") || strstr(format, "🔍")) priority = ANDROID_LOG_DEBUG;
