...
#define WWNLog(module, fmt, ...) do { \
    ...
    fprintf(stderr, "%04d-%02d-%02d %02d:%02d:%02d [%s] %s\n", \
        _wtm.tm_year + 1900, _wtm.tm_mon + 1, _wtm.tm_mday, \
        _wtm.tm_hour, _wtm.tm_min, _wtm.tm_sec, \
        module, [_wmsg UTF8String]); \
} while(0)
...
#define WWNLogFd(fd, module, fmt, ...) do { \
    time_t _wt = time(NULL); \
    struct tm _wtm; \
    localtime_r(&_wt, &_wtm); \
